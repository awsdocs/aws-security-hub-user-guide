...
The overall status for each control is based on the value of `Compliance.Status` for the active findings for that control\. For the administrator account, the overall status includes the active findings in the administrator account and the member accounts\.
...
### Security score calculation for administrator accounts<a name="standard-security-score-admin"></a>

For the administrator account, the score for a standard reflects both the administrator account and all of the member accounts\. The calculation process for a administrator account is different from the process for a member account\. This is to ensure that the score for the administrator account includes controls that are enabled for member accounts but disabled for the administrator account\.
