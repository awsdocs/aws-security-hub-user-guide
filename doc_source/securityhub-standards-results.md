...
When it runs checks against the enabled controls for the enabled security standards, AWS Security Hub generates findings\. These findings use the AWS Security Finding Format \(ASFF\)\. Note that if the finding size exceeds the maximum of 240 KB, then the `Resource.Details` object is removed\. For findings for controls that use AWS Config rules, you can view the resource details on the AWS Config console\.
...
For example, the AWS Config rule `iam-password-policy` is used by multiple controls in the CIS AWS Foundations Benchmark standard and by IAM\.7 in the Foundational Security Best Practices standard\. Each time Security Hub runs a check against that AWS Config rule, it generates a separate finding for each related control, but only charges once for the check\.

## Control\-related information in the ASFF<a name="securityhub-standards-results-asff"></a>

In addition to the required information about the finding and its associated resource, findings generated from security checks contain the following control\-specific information\.

### Compliance<a name="control-findings-asff-compliance"></a>

For findings generated by security checks, the [`Compliance`](securityhub-findings-format-attributes.md#asff-compliance) field in the ASFF contains the control\-related findings details\. The [`Compliance`](securityhub-findings-format-attributes.md#asff-compliance) field includes the following information\.
...
The following table lists the available status reason codes and descriptions\.
...
### ProductFields<a name="control-findings-asff-productfields"></a>
...
The overall status for each control is based on the value of `Compliance.Status` for the active findings for that control\. For the master account, the overall status includes the active findings in the master account and the member accounts\.
...
On the **Security standards** page, each enabled standard displays a security score, which is between 0% and 100%\. The standard details page also displays the overall security score\.
...
### Security score calculation for master accounts<a name="standard-security-score-admin"></a>

For the master account, the score for a standard reflects both the master account and all of the member accounts\. The calculation process for a master account is different from the process for a member account\. This is to ensure that the score for the master account includes controls that are enabled for member accounts but disabled for the master account\.

The process is as follows:

1. For each control, Security Hub gets all of the active findings across all of the accounts\.

   Security Hub does not check whether the control is enabled or disabled in each account\. If a control has active findings, then Security Hub treats it as enabled\.

   If an account disabled a control within the previous 24 hours, then the score might include findings for that control\.

1. Security Hub uses those findings to determine the overall status of each control across all of the accounts\.

1. Security Hub uses those control statuses to determine the overall security score\.
